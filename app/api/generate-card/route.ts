...
Use the full Google-inspired palette: black, white, warm gray, Google Blue #4285f4, Google Red #ea4335, Google Yellow #fbbc04, and Google Green #34a853.
Balance visible blue, red, yellow, and green pixel clusters across the portrait and background; do not let the image collapse into only yellow, black, and gray.
If a style-reference image is provided, use it only as palette and 16-bit rendering guidance, not as a replacement identity.
...
export async function POST(request: Request) {
  ...
  if (!isEventGateOpen(request)) {
    return jsonResponse(
      {
        error: "La generación estará disponible el 16 de mayo.",
      },
      { status: 403 },
    );
  }
